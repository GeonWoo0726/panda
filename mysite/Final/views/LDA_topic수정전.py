    ...
    if request.method == 'POST':
        ...
        def extract_nouns(text):
            ...
            stopwords = ['아', '휴', '아이구', '아이쿠', '아이고', '어', '나', '우리', '저희', '따라', '의해', '을', '를', '에', '의', '가', '으로',
                         ...
                         '아야', '앗', '아야', '콸콸', '졸졸', '좍좍', '뚝뚝', '주룩주룩', '솨', '우르르', '그래도', '또', '그리고', '바꾸어말하면',
                         ...
                         '방법', '단계']  # 불용어 리스트
            return [token.form for token in kiwi.tokenize(text) if token.tag in {'NNG', 'NNP'} and token.form not in stopwords and len(token.form) > 1]
